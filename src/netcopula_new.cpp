...
double mudelta_logpost(const arma::vec& mudelta, const arma::mat& delta_arma, const arma::mat& y, const arma::mat& n, const arma::mat& x, const Rcpp::IntegerVector& baseline, const Rcpp::IntegerVector& trt, const Rcpp::IntegerVector& study, const Rcpp::IntegerVector& narms, const arma::mat& d, const arma::mat& Sigma_M, const Rcpp::List& Gamma, const int& m, const double& mu_sigma, const double& eps, const double& eps_ab) {
  ...
  for (unsigned int ik = 0; ik < n_datapoints; ik++) {
    ...
    for (unsigned int h = 0; h < n_datapoints; h++) {
      if ((study(h) == study(ik)) && (baseline(h) == trt(h))) {
        mu = mudelta(h);
        break;
      }
    }
    ...
    // Rprintf("ik = %d - mu = %4.6f - delta = %4.6f\n", ik, mu, delta);
...
Rcpp::List nc_mcmc_mh_new(const Rcpp::RObject& data, const Rcpp::List& init, const int& totiter, const Rcpp::List& prior, const Rcpp::List& prop, const Rcpp::List& tuning, const Rcpp::List& adapt, const bool& verbose) {
  ...
  double ran_unif, mudelta_rate, Gamma_rate, d_rate, Sigma_M_rate;
  ...
  arma::mat mudelta(n_datapoints, M);
  arma::cube mudelta_chain(n_datapoints, M, totiter);
  for (unsigned int ik = 0; ik < n_datapoints; ik++) {
    for (unsigned int m = 0; m < M; m++) {
      if (baseline(ik) == trt(ik)) {
        mudelta(ik, m) = mu_long(ik, m);
      } else {
        mudelta(ik, m) = delta(ik, m);
      }
    }
  }
  ...
  arma::vec rho_mudelta(M);
  rho_mudelta.fill(2.38/sqrt(n_datapoints));
  arma::vec mudelta_curr(n_datapoints), mudelta_prop(n_datapoints);
  double mudelta_A = 0.0, mudelta_B = 0.0, target_mudelta_curr = 0.0, target_mudelta_prop = 0.0;
  int accept_mudelta = 0;
  arma::mat mu_mudelta(n_datapoints, M);
  arma::vec mu_mudelta_vec(n_datapoints);
  arma::mat theta_mudelta(n_datapoints, every);
  arma::mat ar_mudelta_vec(M, totiter);
  arma::mat ar_mudelta(2, M, arma::fill::ones);
  arma::vec ar_mudelta_sub(2, arma::fill::zeros);
  Rcpp::List prop_mudelta(4);
  arma::cube cov_mudelta(n_datapoints, n_datapoints, M);
  for (unsigned int m = 0; m < M; m++) {
    cov_mudelta.slice(m) = arma::eye(n_datapoints, n_datapoints);
  }
  arma::mat cov_mudelta_mat(n_datapoints, n_datapoints);
  ...
  while (niter < totiter) {
    ...
    for (unsigned int m = 0; m < M; m++) {
      // updating mu (study-specific baseline effect) and delta (study-specific [random] treatment effects), separately for each outcome
      mudelta_curr = mudelta.col(m);
      mudelta_prop = arma::vectorise(rmvt_arma(1, mudelta_curr, pow(sd_multplier*rho_mudelta(m), 2)*cov_mudelta.slice(m), 7));

      target_mudelta_curr = mudelta_logpost(mudelta_curr, delta_arma, y_imp_arma, n_imp_arma, x_imp_arma, baseline, trt, study_index, narms_study, d, Sigma_M, Gamma, m, mu_sigma, eps, eps_ab);
      target_mudelta_prop = mudelta_logpost(mudelta_prop, delta_arma, y_imp_arma, n_imp_arma, x_imp_arma, baseline, trt, study_index, narms_study, d, Sigma_M, Gamma, m, mu_sigma, eps, eps_ab);

      mudelta_A = target_mudelta_prop - target_mudelta_curr;
      // mudelta_B dovrebbe essere la differenza della proposal distribution,
      // ma e' simmetrica, quindi sparisce; la prior e' gia' dentro la
      // target
      mudelta_B = 0.0;
      mudelta_rate = exp(mudelta_A + mudelta_B);
      ran_unif = R::runif(0.0, 1.0);
      if (!ISNAN(target_mudelta_prop) && !ISNAN(target_mudelta_curr)) {
        if (ran_unif < mudelta_rate) {
          mudelta.col(m) = mudelta_prop;
          for (unsigned int ik = 0; ik < n_datapoints; ik++) {
            if (baseline(ik) == trt(ik)) {
              mu(study_index(ik) - 1, m) = mudelta(ik, m);
              delta(ik, m) = 0.0;
            } else {
              delta(ik, m) = mudelta(ik, m);
            }
          }
      ...
      ar_mudelta_vec(m, niter) = fmin(mudelta_rate, 1);
    ...
    mudelta_chain.slice(niter) = mudelta;
    ...
    if (((adapt_k + 1) <= maxiter) && (((niter + 1) % every) == 0)) {
      ...
      for (unsigned int m = 0; m < M; m++) {
        if (fabs(ar_mudelta(0, m) - tar) > tol) {
          theta_mudelta = mudelta_chain.subcube(0, m, niter - every + 1, n_datapoints - 1, m, niter);
          if (adapt_k == 0) {
            // means by rows
            mu_mudelta.col(m) = arma::mean(theta_mudelta, 1);
          }
          mu_mudelta_vec = mu_mudelta.col(m);
          tmp = arma::vectorise(ar_mudelta_vec.submat(m, niter - every + 1, m, niter));
          ar_mudelta(1, m) = arma::as_scalar(arma::mean(tmp));
          ar_mudelta_sub(0) = ar_mudelta(0, m);
          ar_mudelta_sub(1) = ar_mudelta(1, m);
          cov_mudelta_mat = cov_mudelta.slice(m);
          prop_mudelta = rwmh_adapt(theta_mudelta, mu_mudelta_vec, rho_mudelta(m), cov_mudelta_mat, ar_mudelta_sub, alpha, beta, gamma, tar, adapt_k, true, 2, false);
          rho_mudelta(m) = Rcpp::as<double>(prop_mudelta["rho"]);
          cov_mudelta_mat = Rcpp::as<arma::mat>(prop_mudelta["covariance"]);
          if (!is_positive_definite(cov_mudelta_mat, 560)) {
            cov_mudelta_mat = make_positive_definite(cov_mudelta_mat);
          }
          cov_mudelta.slice(m) = cov_mudelta_mat;
          mu_mudelta_vec = Rcpp::as<arma::vec>(prop_mudelta["mu"]);
          mu_mudelta.col(m) = mu_mudelta_vec;
          ar_mudelta(0, m) = Rcpp::as<double>(prop_mudelta["ar"]);
        }
      }
    ...
    if ((((niter + 1) % print_every) == 0) && verbose) {
      Rprintf("   iter. %d/%d ==> d: %1.3f - Gamma: %1.3f - mu/delta: %1.3f - Sigma_M: %1.3f\n", (niter + 1), totiter, accept_d/(static_cast<double>(totiter)), arma::mean(accept_Gamma_q)/(static_cast<double>(totiter)), accept_mudelta/(static_cast<double>(totiter*M)), accept_Sigma_M/(static_cast<double>(totiter)));
    }
  ...
  Rcpp::List accept = Rcpp::List::create(
    ...
    Rcpp::Named("mudelta") = accept_mudelta/static_cast<double>(totiter*M),

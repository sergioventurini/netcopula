...
data(alcoholdependence)
id_arms <- match(unique(alcoholdependence[, "study"]),
  alcoholdependence[, "study"])
id <- data.frame(studyid = unique(alcoholdependence$studyid), study = 1:41,
  narms = alcoholdependence[id_arms, "narms"],
  baseline = alcoholdependence[id_arms, "baseline"])
M <- 3
ref_trt <- 1
nc_data <- new("nc_data",
  type = "binary",
  ...
  n_treatments = 4,
  study_id = id,
  study_data = alcoholdependence[, 2:12],
  ref_trt = ref_trt)
...
# prior parameters
prm.prior <- nc_prior(mu_sigma2 = 10^3, d_sigma2 = 10^3, beta_sigma = 10^(-1/8),
  nGamma = nGamma, eta_prior = 1)
